 ...
 config.vm.box_check_update = false
      ...
      servers_list.each do |server|
        config.vm.define server["vagrant_box_host"] do |box|
          ...
          box.vm.network "forwarded_port", guest: server["guest_port"], host: server["host_port"], id: 'moloch'
          box.vm.network "forwarded_port", guest: server["guest_port1"], host: server["host_port1"], id: 'ELK'
